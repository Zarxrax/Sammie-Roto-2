...
class StableVideoDiffusionPipelineOutput(BaseOutput):
    ...
    Args:
        frames (`[List[List[np.ndarray]]`, `np.ndarray`, `torch.Tensor`]):
            List of denoised RGB uint8 frames of length `batch_size` or numpy array or torch tensor of shape
            `(batch_size, num_frames, height, width, num_channels)`.
    """
    frames: Union[List[List[np.ndarray]], np.ndarray, torch.Tensor]
...
class StableVideoDiffusionPipelineWithMask(DiffusionPipeline):
    ...
    def _get_zero_image_embeddings(
            self,
        ...
        """
        Returns a zero tensor of the correct shape for encoder_hidden_states.
        CLIP conditioning is not used by this fine-tuned model; the UNet was
        trained with zeroed image embeddings, so we construct them directly.
        """
        hidden_size = self.unet.config.cross_attention_dim
        return torch.zeros((batch_size, 1, hidden_size), dtype=dtype, device=device)
    ...
    def check_inputs(self, image, height, width):
        if (
            ...
            raise ValueError(f"`image` has to be of type `torch.Tensor`, OpenCV `np.ndarray`, or `list` but is {type(image)}")
    ...
    def __call__(
            self,
            image: Union[List[np.ndarray], torch.Tensor],
            mask_image: Union[List[np.ndarray], torch.Tensor],
            alpha_matte_image: Optional[Union[List[np.ndarray], torch.Tensor]] = None,
        ...
        image_embeddings = self._get_zero_image_embeddings(device, dtype, batch_size * num_videos_per_prompt)
        ...
        added_time_ids = self._get_add_time_ids(
            fps, motion_bucket_id, noise_aug_strength, dtype, batch_size, num_videos_per_prompt
        )
...
class StableVideoDiffusionPipelineWithCrossAtnnMask(DiffusionPipeline):
    ...
    def __call__(
            self,
            image: Union[np.ndarray, torch.Tensor],  # Static image for appearance
            mask_image: List[np.ndarray],  # Video mask for motion
            height: int = 576,
            width: int = 1024,
            num_frames: Optional[int] = None,
